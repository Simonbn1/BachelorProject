...
import { fetchMyTimesheets, fetchTimeEntries } from "../api/timesheetsApi.ts";
...
export type TimesheetStatus = "NOT_SENT" | "SENT" | "APPROVED" | "REJECTED";
...
export function useTimesheet(initialWeekStart?: string | null) {
  ...
  useEffect(() => {
    async function loadTimesheetStatus() {
      try {
        const timesheets = await fetchMyTimesheets();
        const current = timesheets.find((t) => t.weekStart === weekStart);

        setTimesheetStatus(current?.status ?? null);
      } catch (error) {
        console.error("Kunne ikke hente timesheet-status:", error);
    ...
    async function loadEntries() {
      try {
        ...
        for (const entry of entries) {
          ...
          loadedWorkItems.set(workItemId, {
            projectId,
            title: workItemTitle,
          });
        }
  ...
  function getRowTotal(workItemId: number) {
    ...
    return days.reduce((sum, day) => {
      return sum + getNumericValue(workItemId, day);
    }, 0);
  }

  const weekTotal = visibleProjects.reduce((sum, project) => {
    return sum + getRowTotal(project.workItemId);
  }, 0);
  ...
  const isLocked = timesheetStatus === "SENT" || timesheetStatus === "APPROVED";
